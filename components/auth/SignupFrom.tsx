...
async function simulateSignup(values: SignupFormValues): Promise<{ ok: boolean; message?: string }> {
    await new Promise((resolve) => setTimeout(resolve, 1200));

    // fake "already taken" case so you can see the error state
...
export function SignupForm() {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await simulateSignup(result.data);
            if (response.ok) {
                toast.success("تم إنشاء الحساب بنجاح!");
            } else {
                toast.error(response.message ?? "حدث خطأ أثناء إنشاء الحساب");
            }
        } catch {
    ...
    return (
        <div className="w-full md:p-5 flex flex-col justify-between">
            <div className="bg-white rounded-xl p-5 mt-10">
                <h1 className="text-[20px] md:text-[34px] font-bold text-[#171A1F] text-center md:text-right"> إنشاء حساب جديد  </h1>
                ...
                <form onSubmit={handleSubmit} noValidate className="mt-8 space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            className={cn("ctm-inp", fieldErrors.businessName && "border-red-500 focus-visible:ring-red-500")}
                        />
                        {fieldErrors.businessName && (
                            <p className="text-sm text-red-600">{fieldErrors.businessName}</p>
                        )}
                    </div>
                ...
                <p className="mt-6 text-center md:text-[20px] text-[#1E1F1F]">
                    لديك حساب بالفعل؟{" "}
                    <Link href="/login" className="font-medium text-[#322A7C] hover:underline">
                        تسجيل الدخول
                    </Link>
                </p>
            </div>
