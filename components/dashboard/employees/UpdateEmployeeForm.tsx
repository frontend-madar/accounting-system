...
import { useUpdateEmployee, useEmployeeById } from "@/hooks/use-employee";
...
function UpdateEmployeeFormSkeleton() {
    return (
        <div className="space-y-8 pt-2">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-border pt-5">
                <Skeleton className="h-11 w-[130px] rounded-md" />
...
export function UpdateEmployeeForm({ employeeId, open, onOpenChange }: UpdateEmployeeFormProps) {
    const fileInputRef = useRef<HTMLInputElement>(null);
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const { mutate: updateEmployee, isPending } = useUpdateEmployee();

    const { data: employee, isLoading: isEmployeeLoading } = useEmployeeById(
    ...
    // Reset any leftover file selection when switching to a different employee.
    useEffect(() => {
