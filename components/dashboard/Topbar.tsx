...
export function Topbar({
    ...
    return (
        <header
            className={cn(
                "flex items-center justify-between gap-4 rounded-2xl  shadow-[0px_3px_10.3px_0px_#0000001A] bg-white px-5 py-3",
                className
            )}
        >
            {!path && <h1 className="shrink-0 text-[24px] font-semibold text-[#0F1219]">{title}</h1>}
            ...
            {path && (
                <div className="flex items-center gap-3" >
                    <div className="flex items-center gap-1" >
                        ...
                        <Link href={'/dashboard'} className="text-[#B1B2B4] text-[18px]"> الرئيسية  </Link>
                    </div>

                    <span> <ChevronLeft className="text-[#161616]" /> </span>
                    <span className="text-[#0F1219] text-[18px]" > {path} </span>
                </div>
            )}
                ...
                {!path && search && (
                    <div className="relative ">
                        ...
                        <Input
                            ...
                            className="h-10 w-[300px] rounded-full border-border bg-muted/40 pr-9 text-right"
                        />
                    </div>
                )}
